...
buildScript/ 工程打包时用的打包脚本存放目录，不会打包进安装包
buildScript/main.lua 打包脚本入口文件，即第1个运行的打包脚本文件
...
其他
  ...
  手册：离线版lua官方手册
  联系作者(这个是Androlua的，不是本软件ZAndrolua的)：加入官方qq群与作者交流
...
@
 @XA专属插件API@
@
以下<...>表示匿名函数和匿名函数的参数的含义。

xapluglib.addMenu(菜单选项名称,<>)
添加标题栏菜单选顶

xapluglib.getOpenProject()
返回当前打开项目

xapluglib.isdebug()
判断当前是否开启调试台，如果是则返回true。

xapluglib.getOpenFile()
返回当前正在打开的文件全路径。

xapluglib.setBuildClick(<要打包的项目路径>)
设置打包时执行函数。

xapluglib.setSaveClick(<要保存的文件全路径>)
设置保存文件时执行函数

xapluglib.setRunClick(<运行的文件路径,当前打开项目>)
设置运行文件时执行函数。
